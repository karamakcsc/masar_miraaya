...
def create_item_group():
    ...
    try:
        ...
        all_categories = magento_categories['children_data']
        for category in all_categories:
            process_category(category, root_category)
...
def get_magento_sales_invoices():
    try:
        ...
        if "items" in json_response and len(json_response["items"]) > 0:
            ...
            customer_id = magento_invoice.get("order_id")

            customer_details = get_magento_customer_details(customer_id)
            ...
            for item in magento_invoice["items"]:
                ...
                product_details = get_magento_product_details(sku)
                ...
                if product_details:
                    data["items"].append({
                        ...
                        "image": upload_image_to_frappe(product_details["image"], product_details["item_code"]) if product_details["image"] else ''
                    })

            response = requests.post(f"{base_url_frappe_sales_invoice}/Sales Invoice", headers=headers_frappe_sales_invoice, json=data)
            response.raise_for_status()

            return "Sales invoice created successfully in Frappe."
    except Exception as e:
        frappe.throw(f"Error creating sales invoice: {e}", "Magento Sync")
        return f"Error creating sales invoice: {e}"
...
def create_sales_order():
    ...
    for order in sales_order:
        ...
        for item in order['items']:
            ...
            response = requests.post(f'{base_url}/Sales Order', headers=headers, json=payload)
