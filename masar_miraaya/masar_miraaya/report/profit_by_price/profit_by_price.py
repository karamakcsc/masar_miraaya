	...
	columns.extend(
		[
			...
			},
			{
				...
				"fieldtype": filters.valuation_field_type,
				"options": "Company:company:default_currency" if filters.valuation_field_type == "Currency" else None,
				"convertible": "rate",
			},
			{
...
def get_stock_ledger_entries(filters, items):
	...
	query = (
		frappe.qb.from_(sle)
		.left_join(ip)
		.on((sle.item_code == ip.item_code) & (ip.price_list == price_list))
		.select(
			sle.item_code,
			...
			sle.incoming_rate,
			sle.valuation_rate,
			ip.price_list_rate.as_("item_price"), 
			sle.company,
			sle.voucher_type,
